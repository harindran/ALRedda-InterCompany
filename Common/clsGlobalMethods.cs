...
namespace General.Common
{
    class clsGlobalMethods
    {
        ...
        public string getSingleValue(string StrSQL)
        {
            ...
            {
                clsModule.objaddon.objglobalmethods.WriteErrorLog(" Get Single Value Function Failed :  " + ex.Message + StrSQL);
                return "";
            }
        }
       ...
       public  bool CheckIfColumnExists(DataTable dataTable, string columnName)
        ...
        public double Cton(object Pstring)
        {
            double Cton = 0;
            try
            {
                ...
                if (double.TryParse(Lstr, out LdblResult))
                    Cton = LdblResult;
                return Cton;
            }
            catch (Exception ex)
        ...
        public string GetColumnValue(SAPbouiCOM.Grid matrix, int Index)
        {            
            for (int i = 0; i <= matrix.Columns.Count - 1; i++)
            {
                if (i== Index)
                {
                    return matrix.Columns.Item(i).UniqueID;
                }
            }
            return "";
        }
        public int Getcomboboxindex(SAPbouiCOM.ValidValues ValidValues, string value)
        {
            int selectedIndex = -1;
            for (int i = 0; i < ValidValues.Count; i++)
            {

                if (ValidValues.Item(i).Value.Equals(value))
                    selectedIndex = i;
            }
